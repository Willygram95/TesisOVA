<?php
	...
	$where = "";
	$sql = "SELECT * FROM ovaglosario $where ORDER BY $orden ASC";
...
<html lang="es">
	<head>
		<title><?php echo $_SESSION["id_asignatura"];?>: Administración de Glosario</title>
	...
	<body class="bodyadmin">
		...
		<div class="container">
			<div class="row areasuperioradmin">
				<div class="col-lg-4 col-md-4 col-sm-2 text-left">
					<div class="row">
						<div class="col-lg-4">
							<a href="registrarglosario.php" class="btn botonnuevo btnseguntam">Nuevo término</a>
						</div>	
					</div>
				</div>
			</div>
		...
		<div class="container">
			<div class="contenedortabla">
				<div class="row table-responsive tablacentrada">
					<table class="table table-striped">
						...
						<tbody id="tfilas">	
						<?php $numero_filas = $resultado->num_rows; if($numero_filas<=0){echo "<tr><td class='listadovacio'>El listado está vacío</td><td></td><td></td><td></td></tr>";}else{?>	
							...
							<tr>								
								...
								<td><a href="modificarglosario.php?id=<?php echo $row['id']; ?>" data-toggle="tooltip" title="Editar término"><span class="glyphicon glyphicon-pencil"></span></a></td>
								<td><span data-toggle="tooltip" title="Eliminar término"><a href="#" data-href="eliminarglosario.php?id=<?php echo $row['id']; ?>" data-toggle="modal" data-target="#confirm-delete"><span id="fila<?php echo $nfila; ?>" class="glyphicon glyphicon-trash"></span></a></span></td>
							</tr>
							<script>cborrar[nfilaj]="<?php echo $row['termino'];?>"; nfilaj=nfilaj+1;</script>
							<?php $nfila=$nfila+1;} } ?>
						</tbody>
					</table>
				</div>
			...
			<h4 class="modal-title" id="myModalLabel">Eliminar término</h4>
			</div>
		...
		<div class="modal fade" id="mdayuda" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body">
						<p ALIGN="justify">Esta área se encarga de <b>Administración de glosario</b>, el cual consiste en editar los términos del OVA.</p>

						      <p ALIGN="justify">Al darle clic en <b>Nuevo término</b>, luego llenará el formulario correspondiente.</p>

						      <p ALIGN="justify">Pase el cursor por los iconos de edición de los términos registrado, luego éste le muestra el significado o su función. </p>
					</div>
				</div>
			</div>
		</div>			
			...
			$('#tfilas').click(function(e){
				    ...
				    document.getElementById("elmsj").innerHTML = "¿Desea eliminar el término <strong>"+cborrar[res[1]]+"</strong>?";
			});		
			</script>	
			<!-- <script>
