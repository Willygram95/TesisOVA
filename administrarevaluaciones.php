<?php
	...
	$sqltc = "SELECT * FROM ovaseleccion WHERE id_tema='$cualtm' AND categoria=2 ORDER BY $orden ASC";
...
<html lang="es">
	<head>
		<title><?php echo $_SESSION["id_asignatura"];?>: Administración de Evaluaciones</title>
			...
			<div class="row areasuperioradmin">
				...
				<div class="row">			
					<div class="col-lg-4 col-md-4 col-sm-2 col-xs-12 text-left colseguntam">
						<div class="row">
							<div class="col-lg-12">
								<a href="registrarevaluacion.php?id=<?php echo $row['id']; ?>" class="btn botonnuevo btnseguntam">Nueva pregunta</a>
							</div>	
						</div>
					</div>
					...
					<div class="col-lg-8 col-md-8 col-sm-10 text-right colseguntam">
						<div class="row">		
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right colseguntam">	
								<a href="ordenar.php?qo=ev&ct=<?php echo $cualtm; ?>&to=<?php echo $row['titulo']; ?>" class="btn botonbuscar btnseguntam">Ordenar preguntas</a>
							</div>
						</div>
					</div>					
					<?php } ?>					
			...
			<div class="container contenedortabla">			
				<div class="row table-responsive tablacentrada">
					<table class="table table-striped">
						<thead>
							<tr>
								<th><a href="administrarevaluaciones.php?orden=enunciado&id=<?php echo $cualtm;?>">Enunciado</a></th>
								<th><a href="administrarevaluaciones.php?orden=tipo&id=<?php echo $cualtm;?>">Tipo</a></th>
								<th><a href="administrarevaluaciones.php?orden=numopciones&id=<?php echo $cualtm;?>">Número de opciones</a></th>
								<th><a href="administrarevaluaciones.php?orden=valor&id=<?php echo $cualtm;?>">Valor</a></th>
								<th class="colstatus"><a href="administrarevaluaciones.php?orden=status&id=<?php echo $cualtm;?>">Status</a></th>
								<th></th>
								<th></th>
							</tr>						
						</thead>
						...
						<?php $numero_filas = $resultadotc->num_rows; if($numero_filas<=0){echo "<tr><td class='listadovacio'>El listado está vacío</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>";}else{?>	
							<?php $nfila=1; while($rowtc = $resultadotc->fetch_array(MYSQLI_ASSOC)) { ?>	
								<tr>		
									...
									<td><?php echo $rowtc['valor']; ?></td>
									<td class="colstatus"><a href="cambiarstatusevaluacion.php?id=<?php echo $rowtc['id']; ?>&tm=<?php echo $cualtm; ?>"><span data-toggle="tooltip" title="<?php if($rowtc['status']==0){echo 'Pregunta inactiva';}else{echo 'Pregunta activa';}?>" class=<?php if($rowtc['status']==0){echo '"glyphicon glyphicon-ban-circle"';}else{echo '"glyphicon glyphicon-ok-circle"';}?>></span></a></td>
									<td><a href="modificarevaluacion.php?id=<?php echo $rowtc['id']; ?>&tm=<?php echo $cualtm; ?>" data-toggle="tooltip" title="Editar pregunta"><span class="glyphicon glyphicon-pencil"></span></a></td>
									<td><span data-toggle="tooltip" title="Eliminar pregunta"><a href="#" data-href="eliminarevaluacion.php?id=<?php echo $rowtc['id']; ?>&tm=<?php echo $cualtm; ?>" data-toggle="modal" data-target="#confirm-delete"><span id="fila<?php echo $nfila; ?>" class="glyphicon glyphicon-trash"></span></a></span></td>
								</tr>								
							<?php $nfila=$nfila+1;} } ?>
						</tbody>
					</table>
		...
		<div class="modal fade" id="mdayuda" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body">
						<p ALIGN="justify">Esta área se encarga de <b>Administración de evaluaciones</b>, el cual consiste en editar las actividades del OVA.</p>

						      <p ALIGN="justify">Al darle clic en <b>Nueva pregunta</b>, luego se llenará el formulario correspondiente.</p><br/>
						      <p ALIGN="justify">El área central de la página se muestra la serie de actividades registrada.</p><br/>

						      <p ALIGN="justify">Pase el cursor por los iconos de edición y este le muestra el significado o su función. </p>
						      <p ALIGN="justify">El botón <b>Ordenar preguntas</b>, permite organizar las actividades por un orden determinado. </p><br/>
